...
public class SelectionListener extends AdapterImpl {
    ...
    boolean setEditFeature( Layer layer ) {
        ...
        if (layer.getProperties().get(PROP_KEY) != null
                && layer.getProperties().get(PROP_KEY) instanceof SimpleFeature) {
            editManager.setEditFeature((SimpleFeature) layer.getProperties().get(PROP_KEY), layer);
            return true;
        }
        ...
        try {
            ...
            FeatureCollection<SimpleFeatureType, SimpleFeature>  results = store.getFeatures(new DefaultQuery(layer.getSchema()
			...
			if (map.getContextModel() != null) {
				...
				for (Layer layer : map.getLayersInternal()) {
					if (layer.isType(FeatureStore.class)) {
						try {
							FeatureStore<SimpleFeatureType, SimpleFeature> featureStore = layer.getResource(FeatureStore.class, null);
