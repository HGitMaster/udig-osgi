...
/**
 * Basic WMS-C Renderer.  Determines what tiles from which zoom level need
 * to be retrieved from the service; retrieves these tiles; and draws them
 *
...
public class BasicWMSCRenderer extends RendererImpl implements IRenderer {
    ...
    public void render( Graphics2D graphics, IProgressMonitor monitor ) throws RenderException {
        ...
        IGeoResource handle = getContext().getLayer().findGeoResource(WMSTileSet.class);
        
        try{
            TileSet tileset = handle.resolve(WMSTileSet.class, new SubProgressMonitor(monitor, 30));
            TiledWebMapServer server = handle.resolve(TiledWebMapServer.class, new SubProgressMonitor(monitor, 30));
            ...
            if (!bounds.getCoordinateReferenceSystem().equals(tileset.getCoordinateReferenceSystem())){
                ...
                MathTransform transform = CRS.findMathTransform(bounds.getCoordinateReferenceSystem(), tileset.getCoordinateReferenceSystem());
                bounds = new ReferencedEnvelope(JTS.transform(bounds, transform), tileset.getCoordinateReferenceSystem());
            }

            // determine scale factor used to determine zoom level
            //compute the scale factor based on the viewport size; we cannot use the bounds and tile size becuase the 
            //bounds may not be the full size of the tile and we might get the wrong scale
            double scaleFactor = getContext().getViewportModel().getBounds().getWidth() / getContext().getMapDisplay().getWidth();

            // create a TileRange to handle loading the tiles
            com.vividsolutions.jts.geom.Envelope bnds = new com.vividsolutions.jts.geom.Envelope(bounds.getMinX(), bounds.getMaxX(), bounds.getMinY(), bounds.getMaxY());
    ...
    private void renderTile(Graphics2D graphics, Tile tile, CoordinateReferenceSystem crs, RasterSymbolizer style) throws FactoryException, TransformException {
        ...
        GridCoverage2D coverage = (GridCoverage2D) factory.create("GridCoverage", tile.getBufferedImage(), ref); //$NON-NLS-1$        
        ...
        try{
            ...
            t.printStackTrace();
            WmsPlugin.log("Error Rendering tile. Painting Tile:" + (coverage != null ? coverage.getName() : ""), t); //$NON-NLS-1$ //$NON-NLS-2$
        }
    }
