        ...
        ExtensionPointUtil.process(getDefault(),
                "net.refractions.udig.catalog.ICatalog", new ExtensionPointProcessor(){ //$NON-NLS-1$
                    public void process( IExtension extension, IConfigurationElement element )
                            throws Exception {
                        cats.add((ISearch) element.createExecutableExtension("class")); //$NON-NLS-1$                 
                    }
                });

        ISearch[] ctemp = new ISearch[cats.size() + 1];
        ctemp[0] = catalogs[0];
        Iterator<ISearch> ii = cats.iterator();
