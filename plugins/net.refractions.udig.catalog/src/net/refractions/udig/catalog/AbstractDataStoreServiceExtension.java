...
public abstract class AbstractDataStoreServiceExtension implements
	...
	public String reasonForFailure(Map<String, Serializable> params) {
		if (getDataStoreFactory() == null) {
			throw new NullPointerException("The DataStoreFactory (aka the factory is null). Maybe you forgot to over ride AbstractDataStoreServiceExtension:getFactory()?"); //$NON-NLS-1$
		}					
	 ...
	 * 
	 * @param params
	 *            parameters to be used for creating the datastore
	 * 
	...
	/**
	 * Returns an instance of the factory used to create the DataStore for this
	 * service extension.
	 * <p>
	 * Subclass must provide an instance of this method and can be specific
	 * about the factory they are going to return.
	 * <p>
	 * Example:
	 * <pre><code>
	 ...
	 * </code></pre>
	 * 
	 * @return instance of the factory used to create the Datastore.
	 */
	abstract protected DataAccessFactory getDataStoreFactory();

	private String processParameters(Map<String, Serializable> params,
		...
		for (int i = 0; i < arrayParameters.length; i++) {
			...
			if (!params.containsKey(param.key)) {
				if (param.required) {
					return param.key
							+ Messages.DataStoreServiceExtension_missingKey
			...
			try {
				...
				return Messages.DataStoreServiceExtension_theParam + param.key
						+ Messages.DataStoreServiceExtension_wrongType
						+ param.type
			...
			if (value == null) {
				if (param.required) {
					return param.key
							+ Messages.DataStoreServiceExtension_nullParam
							+ param.description;
				}
			} else {
				if (!param.type.isInstance(value)) {
					return Messages.DataStoreServiceExtension_theParam
							+ param.key
							+ Messages.DataStoreServiceExtension_wrongType
							+ param.type
							+ Messages.DataStoreServiceExtension_butWas
							+ params.get(param.key).getClass(); // value was not
																// of the
																// required type
				}
			}
	...
	/**
	 * For special urls like DB urls. parses out the required information from
	 * the url. Consider:
	 * jdbc.postgis://username:password@hosr:port/database/schema. it will parse
	 * out these parts from the url. In cases like oracle the equivalents have
	 * to be understood.
	 * 
	 * @param url
	 * @return
	...
	protected ParamInfo parseParamInfo(URL url) {
		...
		if (host != null && !"".equals(host)) { //$NON-NLS-1$
			if (host.endsWith("postgis.jdbc")) { //$NON-NLS-1$
				host = host.substring(0, host.length() - 12);
			}
		}
		Integer the_port = url.getPort() == -1 ? new Integer(5432)
				: new Integer(url.getPort());
