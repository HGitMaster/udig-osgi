...
/**
 * Represents one tile in a tileset from a WMS Server.  Has a unique id, bounds, scale and
...
public class WMSTile implements Tile {
    ...
    /**
     * The state of the tile; if something happens while fetching the image and the image
     * isn't returned properly we set the state to error.  If another request is
     * made for this tile we'll try re-requesting the tile.
     */
    private int state = OK;
    
    public WMSTile(TiledWebMapServer server, WMSTileSet tileset, 
    ...
    /**
     * Builds a tile id in the form of:
     * id = "<scale>_<minx>_<maxy>_<maxy>_<miny>"
     *
     * @param bbox
     * @param scale
    ...
    /* (non-Javadoc)
	 * @see net.refractions.udig.catalog.wmsc.server.Tile#loadTile(org.eclipse.core.runtime.IProgressMonitor)
	 */
    public boolean loadTile(IProgressMonitor monitor) {
        ...
        synchronized (lock) {
            ...
            if ((getBufferedImage() != null && getTileState() != WMSTile.INERROR) || monitor.isCanceled()) {
