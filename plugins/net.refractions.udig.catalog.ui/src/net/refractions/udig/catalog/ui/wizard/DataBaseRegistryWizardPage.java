...
/**
 * David's magic superclass reduced to linear, documented, boring reality so
 * that it may be understandable by mortals;
 * 
 * this is an abstract superclass for implementations of the 'IWizardPage'
 * interface required by the 'wizardPage' portion of the
 * <code>net.refractions.udig.catalog.ui.connectionFactory</code> extension
 * point contract with the uDig catalog system.
 * 
 * The connectionFactory extension point needs a DataStore factory to make the
 * connection used by the Catalog and a IWizardPage to obtain from the user the
 * parameters which will be used by the factory.
 * 
 * The IWizardPage interface extends the eclipse RCP 'Import...' system by
 * providing a User Interface SWT Group which will be placed in a 'Page' of the
 * import wizard and will obtain the required parameters to connect to some
 * resource, in this case a database (Db) server. Concrete extensions of this
 * class will then use the parameters to connect to the server using approaches
 * specific to each Database Management System (DBMS).
 * 
 * 
 * WARNING: Modifications to this class must consider the consequences for all
 * extending classes, notably both PostGisWizardPage and
 * AbstractProprietaryDatastoreWizardPage.
 * 
 * 
 * ************* David's magic spells (now for mortals) ***********************
 * 
 * This class performs a number of actions as explained below. These actions
 * have been changed from happening magically in response to users clicking on
 * the widgets to happening after user input or button clicks. Hopefully this
 * logic will be more predicatable, robust, and understandable.
 * 
 * Establish default Connection Parameters: * must be added by the extending
 * class during its construction * are used to set the current connection
 * parameters at the same time * TODO: let the user reset to this in the case
 * where the user is lost
 * 
 * 
 * Get Storage for Connection Parameters: * must be obtained or created by the
 * extending class' constructor.
 * 
 * Get and Use Stored Connection Parameters: * are taken from storage and added
 * to the list by the extending class during its construction. * are added to
 * the drop down list during * TODO: limit the parameters added to a few recent
 * 
 * Retain Successful Connection Parameters: * are added to the list after each
 * successful connection
 * 
 * Store Successful Connection Parameters: * are stored to the storage system
 * after each successful connection
 * 
 * 
 * Show the Advanced group: * by the widgetSelected(.) handler for the check
 * button widget if this widget is clicked (checked).
 * 
 * Activate the lookup and connect buttons: * by the modifyText(.) handler for
 * any of the input widgets, if the current host, port, user, password, and
 * database parameters are all filled out.
 * 
 * Lookup the available databases and schema on the server: * by the
 * widgetSelected(.) handler for the "Lookup" button, when it is clicked.
 * 
 * Connect to a database server: * by the widgetSelected(.) handler for the
 * "Connect" button, when it is clicked.
 * 
 * Activate the finish buttons: * by the widgetSelected(.) handler for the
 * "Connect" button if, after the connection attempt, there is a live connection
 * and the parameter map built from the current connection can be processed by
 * the DataStore factory.
 * 
 * 
 * Reset the lists of available resources when altering key connection params: *
 * by the modifyText(.) handlers for each of the widgets: host, port, user, and
 * password, when they are modified.
 * 
 * 
 * Still to be done: (some of these are distant hopes)
 * 
 * pre-select text on re-edit: * Note this seems to be the default when using
 * the <tab> key to move through the widgets
 * 
 * Hint text completion from the storage list: * Note this requires improvement
 * in the earlier logic. Importantly, we must be able to connect to a resource
 * that is a subset of an earlier resource, i.e. if a previous connection was
 * made to the host "serverAlex," a user must still be able to connect to the
 * host "serverA" by typing 's' 'e' 'r' 'v' 'e' 'r' 'A' '<return>' without our
 * completion logic getting in the way of the user.
 * 
 * Validate input: * This is to be done in three parts, character exclusion, URL
 * component validation, and URL validation: - changes with characters in the
 * 'invalidChar*List' lists will not be allowed (and add an error in the RCP
 * Animated Area). - changes with character sequences in the
 * 'invalidString*List' lists will not be allowed (and add an error in the RCP
 * Animated Area). - URLs will be subject to a coarse verification before being
 * used to connect to a database.
 * 
 * Password entry warnings: * If CapsLock is set during password entry, an error
 * will be placed in the RCP Animated Area.
 * 
 * Connection Status Information: * Will be displayed in the dialog.
 * 
 * Verifications of permissions: * the permissions of the user on the server's
 * database will be obtained and reported to the user.
 * 
 * Verification of the database: * since we can only use spatial tables, the
 * databases offered to the user should be evaluated to ensure they can hold
 * spatial data.
 * 
 * 
 * ****************** Class Outline (i.e. the key pieces) ********************
 * 
 * <pre>
 * 
 ...
 * 
 * 
 * ****************** Contract with Concrete Extending Classses ***************
 * 
 * This class, being abstract, is designed specifically for the needs of the
 * extending classes, notably the PostGISWizardPage and the
 * AbstractProprietaryDatastoreWizardPage classes. See
 * 
 * @see net.refractions.udig.catalog.internal.postgis.ui.PostGisWizardPage
 * 
 * for the cannonical example of how to use (extend) this class in a concrete
 * implementation.
 * 
 * This class implicitly establishes the following contract with its extendors:
 * 
 * I. EXTENDING CLASSES MUST:
 * 
 * Implement a constructor which 1. calls this constructor, idealy with an
 * appropriate string for the title area. 2. gets an IDialogSettings instance
 * from the RCP storage system, creating a new instance if none is available. 3.
 * populates the past connection list 4. populates the db and schema exclusion
 * lists 5. TODO: populates the invalid character and characterSequence lists.
 * 
 * Implement the abstract methods, most importantly the getConnection(.) method
 * that will be used to make a test connection with the host.
 * 
 * Override the createControl(..) method with an initial call to
 * super.createControl(..args..) and then add logic to handle drag-and-drop
 * (TODO).
 * 
 * 
 * II. Extending classes may:
 * 
 * Implement createAdvancedControl(..) to return an SWT Group widget which
 * contains all the input widgets to obtain any DBMS specific advanced
 * parameters desired, and handle all the appropriate event logic that ensues.
 * 
 * Override the getDatabaseResultSet(..) and/or the getSchemasResultSet(..), if 
 * a different technique is necessary to retrieve a list of Databases or Schemas.  
 * ????????????????? DROP THIS SECTION AS REPETITIVE ?????????????????????????
 * Note also:
 * 
 * Extending classes wishing to expand the event handling logic should be aware
 * of the following design for the UI event handling system:
 * 
 * The JDBC 'Connection' to the Db or Schema on the DBMS will be obtained in
 * response to the user clicking on the "Connect" button - This will be
 * performed by the selectionListener(..) method tied to the connectButton. The
 * selectionListener will eventually call the getConnection(..) method assuming
 * the paramters to use are stored in the currConn DataBaseConnInfo field.
 * 
 * The list of databases and schema available to the user on the DBMS will be
 * obtained in response to the user clicking on the "Lookup" button - The
 * selectionListener tied to to the button will create a temporary, dummy
 * connection to the DBMS, possibly using some default database generally
 * available on the specific DBMS if the user has not yet named any more
 * specific database.
 * 
 * The list of available databases and schema will be reset if the user changes
 * any of the key connection parameters - If the user modifies the host, port,
 * user, or password parameters, the list of available databases and schema will
 * be reset to null.
 * 
 * Parameters for successful connections will be stored into the eclipse RCP
 * settings system and retrieved on page construction - Values will be obtained
 * from the eclipse storage system and placed into a java.util.List field. The
 * UI will offer the user a drop down list of past successful values and
 * selection by the user will cause both the values to be stored into the
 * currInfo object and into the text area of the input widgets.
 * 
 * TODO: Input will be verified both character by character during input to the
 * GUI widgets and as overall urls prior to connection - ? should this happen as
 * each widget is about to loose focus? ? There are two strategies to help child
 * classes: either a bunch of new methods can be created with names like
 * verifyHostText() or we can create inner Listener classes.
 * 
 * TODO: When focus arrives on an entry widget, the contents should be
 * pre-selected so that direct input will obliterate existing values - TODO: The
 * system must ignore focus events due to the user moving the mouse around the
 * desktop.
 * 
 * 
 * Auto-completion of values based on previous successful connection. TODO: Make
 * sure the user actively must accept the auto-completed text so that a prior
 * connection to 'localhost' does not prevent a future connection to 'local',
 * i.e. the user should be able to type 'l', 'o', 'c', 'a', 'l', '<return>' and
 * not have the completion hint in the way.
 * 
 * 
 * ****************************************************************************
 * 
...
// TODO: ,VerifyListener
{
	...
	/**
	 * The parameters we will use to make any eventual connection. Parameters
	 * are presumed to be valid once stored in this object. These Parameters
	 * will be obtained via the GUI from the user.
	 */
	protected final DataBaseConnInfo currentDBCI = new DataBaseConnInfo(""); //$NON-NLS-1$
	/**
	 * The parameters which work as defaults for the particular DBMS of the
	 * concrete extender; will be populated during construction.
	 */
	protected final DataBaseConnInfo defaultDBCI = new DataBaseConnInfo(""); //$NON-NLS-1$
	/**
	 * The parameters of any previous successful connection, obtained from
	 * storage during construction of the concrete extending class, added to
	 * following any successful connection and stored prior to exit.
	 * 
	 * Copy new DBCI's into this list with DataBaseConnInfo dbci = new
	 * DataBaseConnInfo(""); storedDBCIList.add(dbci.setParameters(dbciToAdd))
	 * rather than adding the DBCI directly in order to preserve the separation
	 * of instances.
	 * 
	 */
	 ...
	 * This must be here because we need to store the settings on successful
	 * connection.
	 */
	...
	/** 
	 * The connection used to actually connect with the database.  Don't use this 
	 * directly instead call getConnection() and use what it returns.
	 */
    protected Connection realConnection = null;
    
	/**
	 * The name of the section used to store the successful connection parameter
	 * settings in the RCP storage system.
	 */
	...
	// EXCLUSION Lists
	/**
	 * The names of any databases which should not be offered to the user,
	 * generally used for databases that are part of the DBMS itself. The list
	 * will be populated during construction.
	 */
	protected final java.util.List<String> dbExclusionList = new ArrayList<String>();
	/**
	 * The names of any schemata which should not be offered to the user,
	 * generally used for schemata that are part of the DBMS itself. The list
	 * will be populated during construction.
	 */
	 ...
	 * 
	 * @param header
	 *            The string presented to the user in the topmost part of the
	 *            wizard page.
	 */
	...
	// UTILITY METHODS
	/**
	 * Checks if the DBMS uses Schema or not; if true, the schema widget will be
	 * shown and the schema parameter passed to the DBMS during connect.
	 * 
	...
	/**
	 * Checks the database name against those that should not be offered to the
	 * user for connection; by default, all database names are acceptable so the
	 * method always returns false.
	 * 
	 * The constructor of implementing classes should add appropriate names to
	 * the exclusion list.
	 * 
	 * @param d
	 *            the name of the database to check.
	 * @return true if the name should be excluded from those presented to the
	 *         user. Here it will be false always, because by default the
	 *         dbExclusionList is empty.
	 */
	...
	/**
	 * Checks the schema name against those that should not be offered to the
	 * user for connection; by default, all schema names are acceptable so the
	 * method always returns false.
	 * 
	 * The constructor of implementing classes should add appropriate names to
	 * the exclusion list.
	...
	/**
	 * Evaluates if the currentDBCI is complete enough that we could attempt a
	 * connection to lookup values.
	 * 
	 ...
	 * 
	 * TODO: The method currently always returns false; it is probably better to
	 * leave it to the subclass to implement it since the compiler will catch
	 * the missing method.
	 */
	@Override
	...
	public void createControl(Composite comp) {
		...
		if (storedDBCIList.size() > 0) {
			...
			lbl
					.setToolTipText(Messages.DataBaseRegistryWizardPage_recent_tooltip);
			fd = new FormData();
		...
		hostTextWgt
				.setToolTipText(Messages.DataBaseRegistryWizardPage_host_tooltip);
		fd = new FormData();
		fd.top = new FormAttachment(rcntComboWgt, 15);
		...
		portTextWgt
				.setToolTipText(Messages.DataBaseRegistryWizardPage_port_tooltip);
		fd = new FormData();
		fd.top = new FormAttachment(hostTextWgt, 5);
		fd.left = new FormAttachment(0, WIZARD_PARAM_INDENT);
		...
		lbl
				.setToolTipText(Messages.DataBaseRegistryWizardPage_username_tooltip);
		fd = new FormData();
		...
		lbl
				.setToolTipText(Messages.DataBaseRegistryWizardPage_password_tooltip);
		fd = new FormData();
		...
		lookupBtnWgt
				.setText(Messages.DataBaseRegistryWizardPage_button_lookup_text);
		lookupBtnWgt
				.setToolTipText(Messages.DataBaseRegistryWizardPage_button_lookup_tooltip);
		fd = new FormData();
		...
		schemaComboWgt
				.setToolTipText(Messages.DataBaseRegistryWizardPage_schema_tooltip);
		fd = new FormData();
		fd.top = new FormAttachment(dbComboWgt, 5);
		fd.left = new FormAttachment(0, WIZARD_PARAM_INDENT);
		fd.right = new FormAttachment(lookupBtnWgt, -5);
		...
		if (null != advancedGrp) {
			...
			advancedBtnWgt
					.setText(Messages.DataBaseRegistryWizardPage_label_advanced_text);
			advancedBtnWgt
		...
		connectBtnWgt
				.setText(Messages.DataBaseRegistryWizardPage_button_connect_text);
		connectBtnWgt
				.setToolTipText(Messages.DataBaseRegistryWizardPage_button_connect_tooltip);
		fd = new FormData();
	...
	/**
	 * Creates a GUI Group with widgets to set advanced parameters of the
	 * connection or, by default, null if there are no advanced settings.
	 * Classes extending this abstract class may override this method and both
	 * provide the widgets and handle the events.
	 * 
	 * @param arg0
	 *            the parent widget in which the widgets will be added.
	 * 
	 * @return the Group containing the widgets which will be drawn in the
	 *         'Advanced' section of the wizard page or null (the default) if
	 *         there is none.
	 */
	...
	/**
	 * This is the class that will actually obtain the connection. It throws an
	 * SQLException if it's an odd day of the week or if you squint too hard.
	 * 
	 * 
	 * @return A valid connection to a database.  It should not be closed.  
	 * But it might be if you are unlucky.
	 */
	protected abstract Connection getConnection() throws Exception;

	// LOOKUP METHODS
	/**
	 * This method is called in response to selection of the lookup button and
	 * gets the names of the available databases on the DBMS server. 
	 * Instead of overriding this method it is better to override the 
	 * getDatabaseResultSet method if a custom way of getting the database names is 
	 * needed
	 * 
	 * @param con
	 *            the java.sql.Connection returned by getConnection()
	 * 
	 * @return An array of Strings containing the names of only the databases
	 *         available on the server which are suitable for display to the
	 *         user.
	 */
	protected String[] lookupDbNamesForDisplay(Connection con) {
		...
		String[] dbArray = new String[dbList.size()];
    ...
    /**
     * This method can is called so that diffrent databases can provied their own 
     * method of obtaining the list of databases without overriding the lookupDbNamesForDisplay 
     * method.
	 * 
	 * @param c
	 *            the java.sql.Connection returned by getConnection()
	...
	/**
	 * This method is called in response to selection of the lookup button and
	 * gets the names of the available schemata on the DBMS server.
	 * Like lookupDbNameForDisplay it is better to avoid overiding this method
	 * if a database specific implementation is needed.  Instead overide
	 * getSchemaResultSet
	 * 
	 * WARNING: This should never be called if !dbmsUsesSchema().
	 * 
	 * @param con
	 *            the java.sql.Connection object returned by getConnection()
	 * 
	 * @return An array of Strings containing the names of the schemata
	 *         available on the server.
	 * 
	 */
	protected String[] lookupSchemaNamesForDisplay(Connection con) {
		...
		for (int i = 0; i < schemaArray.length; i++) {
			schemaArray[i] = schemaList.get(i);
		}
		return schemaArray;
    ...
    /**
     * This method can is called so that diffrent databases can provied their own 
     * method of obtaining the list of schemas without overriding the lookupSchemasNamesForDisplay 
     * method.
	 * 
	 * @param c
	 *            the java.sql.Connection returned by getConnection()
	...
	/**
	 * The focusGained method is called when input focus passes to one of the
	 * widgets in the page, following the standard FocusListener contract.
	 * 
	 * If we need to distinguish internal changes of focus from changes of focus
	 * involving other programs, such as if the user wanders off to read about
	 * the GNU project in her browser and then comes back, we compare the widget
	 * which lost focus, saved when handling the focusLost event, to the widget
	 * for which this event has been generated. In this case we ignore any
	 * focusGain events for widgets which just had the focus; such widgets
	 * should still be in the correct state.
	 * 
	 * We no longer trigger connections or validation on focusEvents because of
	 * this uncertainty.
	 * 
	 * TODO: Gain of focus on text widgets merely pre-selects the text for
	 * deletion if the user inputs new text.
	 * 
	 * @see org.eclipse.swt.events.FocusListener#focusGained(org.eclipse.swt.events.FocusEvent)
	 * @param e
	 *            the event object describing the action that triggered this
	 *            call
	 */
	...
	/**
	 * The focusLost method is called when input focus leaves one of the widgets
	 * in our page, following the standard FocusListener contract.
	 * 
	 * Note that loss of focus does not necessarily imply the user has to work
	 * on another widget but may be due to a temporary move to another part of
	 * the operating system---the user's mouse could have slipped onto another
	 * window or they could be checking email.
	 * 
	 * We no longer trigger connections or validation on focusEvents because of
	 * this uncertainty.
	 * 
	 ...
	 * @param e
	 *            the event (and widget via e.widget) generated for the focus
	 *            change.
	 */
	...
	/**
	 * The verifyText method is called prior to modification of the text content
	 * in any widget with a Text component, notably the Text and Combo widgets,
	 * following the standard verifyListener contract.
	 * 
	 * TODO: Activate this to make verification work. Re-design it based on
	 * exclusion lists of characters and strings.
	 * 
	 * The method is used to validate user provided input. The user may change
	 * the text character-by-character if they are typing, or may change a whole
	 * string through select then delete, by pasting, or by selecting an entry
	 * in a drop down Combo.
	 * 
	 * The method works by exclusion which will allow sub-classes to become more
	 * restrictive according to their particular rules. This class should
	 * therefore be permissive, excluding only characters which are obviously
	 * wrong. Extending classes can override this method, starting with a call
	 * to this method, i.e. super.verifyText(ve), and then continue with their
	 * own, more restrictive, assessment.
	 * 
	 * @param ve
	 *            the VerifyEvent object representing the action which triggered
	 *            the method call.
	 */
	/*
	 * public void verifyText( VerifyEvent ve) {
	 *  // Host name: alphanum plus . - if (ve.widget == host){ ve.doit = ( ( 0 ==
	 * ve.text.length() ) || Character.isLetterOrDigit(ve.character) || '.' ==
	 * ve.character || '-' == ve.character ); }
	 * 
	 * //Port number: only uses digits and non-characters e.g. bksp or del //
	 * Handle "" also if (ve.widget == port){ ve.doit = ( (0 == ve.text.length() ) ||
	 * Character.isDigit(ve.character) ); return; }
	 * 
	 * //User name: if (ve.widget == user){ return; }
	 * 
	 * //Password is not examined because can be almost anything
	 * 
	 * //Database name: if (ve.widget == database){ return; }
	 * 
	 * //Schema name: if (ve.widget == schema){ return; } }
	 */
	/**
	 * The modifyText method is called by the event handling mechanism for any
	 * widget with a Text field, notably Text and Combo widgets, according to
	 * the standard ModifyListener contract.
	 * 
	 * We use this method to add the modified contents into the currentDBCI. The
	 * text of the modification should already have been verified prior to this
	 * method being called. We may add the verification of the whole text field
	 * either here or in the verify listener.
	 * 
	 * TODO: add a verification step here to establish that the whole text
	 * contents of the widget's Text field are valid. Figure out how to respond
	 * if they are not.
	 * 
	 * If any of the connection parameters are altered, we clear the lists of
	 * available databases and schemata since they are no longer guaranteed to
	 * work but we leave the text as is since the user is working on it.
	 * 
	 * As a last step, we also evaluate if we can now connect, and if so
	 * activate the lookup and connect buttons.
	 * 
	 */
	 ...
	 * not 'default) selection event on any widget to which this class was added
	 * as a SelectionListener.
	 * 
	 * The only widgets we care about are the button widgets. Text and Combo
	 * widgets will receive modifyEvents for any changes to their contents so we
	 * handle their entries in the modifyText(..) method.
	 * 
	 * 
	 ...
	 * 
	 * @param e
	 *            the SelectionEvent which includes e.widget, the widget
	...
	public void widgetSelected(SelectionEvent e) {
		...
		if (e.widget.equals(lookupBtnWgt)) {
			...
			String[] arr = lookupDbNamesForDisplay(con);
			...
			// Logic is in a separate method to be easy to override by extendors
			if (dbmsUsesSchema()) {
				arr = lookupSchemaNamesForDisplay(con);
				if (null != arr) {
			...
			try {
				...
				// Log the error
				CatalogUIPlugin.log(sqlex.getLocalizedMessage(), sqlex);

				// Do nothing since we are trying to close anyhow
			}
		...
		if (e.widget.equals(connectBtnWgt)) {
			...
			passTextWgt.setText(defaultDBCI.getPassString());
			// These two lines don't seem necessary but should be and do no harm
			dbComboWgt.removeAll();
			schemaComboWgt.removeAll();
			if (!excludeDbFromUserChoices(defaultDBCI.getDbString()))
				dbComboWgt.setText(defaultDBCI.getDbString());
			if (null != schemaComboWgt)
				if (!excludeSchemaFromUserChoices(defaultDBCI.getSchemaString()))
	...
	/**
	 * The method called by the event handling mechanism for any 'default'
	 * selection event on any widget to which this class was added as a
	 * SelectionListener.
	 * 
	 * There is nothing to do here. The widgetDefaultSelected method is called
	 * only in the Text and Combo widgets when the <enter> key is typed while
	 * focus is on the Text field. Since changes to the text field all generate
	 * the modifyEvent as well, we will handle modifications in the
	 * modifyText(..) method and can do nothing here.
	 * 
